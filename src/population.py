    ...
    for region in regions:
        ...
        for country in regions[region]:
            if country in country_populations:
                total += country_populations[country]
                countries.append(PopulationTree(
                    False, country, None, country_populations[country]))
        region_trees.append(PopulationTree(False, region, countries, total))
    return region_trees
...
def _get_region_data() -> Dict[str, List[str]]:
    ...
    for i in country_data:
        if i[r][v] in regions and i[r]['id'] != 'NA':
            regions[i[r][v]].append(i['name'])
        elif i[r][v] not in regions and i[r]['id'] != 'NA':
